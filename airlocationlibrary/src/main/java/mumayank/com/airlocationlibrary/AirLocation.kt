...
class AirLocation(
    ...
    private fun addLocationCallback() {
        ...
        locationCallback = object : LocationCallback() {
            ...
            override fun onLocationAvailability(locationAvailability: LocationAvailability?) {
                ...
                if (locationAvailability?.isLocationAvailable == false) {
                    callback?.onFailed(LocationFailedEnum.HighPrecisionNaTryAgainPreferablyWithInternet)
                    fusedLocationClient?.removeLocationUpdates(locationCallback)
                }
            }
        }
        ...
        if (NetworkUtil.isInFlightMode(activityWeakReference.get() as Activity)) {
            callback?.onFailed(LocationFailedEnum.DeviceInFlightMode)
        } else {
    ...
    fun onRequestPermissionsResult(
        ...
        if (requestCode == requestLocation) {
            if (grantResults.isEmpty()) {
                callback?.onFailed(LocationFailedEnum.LocationPermissionNotGranted)
                return
            }
        ...
        if (requestCode == requestCheckSettings) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                if (locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER)) {
                    ...
                    callback?.onFailed(LocationFailedEnum.LocationOptimizationPermissionNotGranted)
                }
            }
        }
    }
